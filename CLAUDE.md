...
**Stato al 2026-08-29**: 15 suite / 114 test (Jest) tutti verdi (`npx jest src`); `npx tsc --noEmit` pulito. Nota: `npm test`/`npx jest` senza scoping raccoglie anche gli spec Playwright sotto `e2e/`, che falliscono a livello di suite per un problema di configurazione pre-esistente e non correlato (Playwright va invocato solo via `npx playwright test`) — usare sempre `npx jest src` per il conteggio delle unità. Il valore precedente (17 suite / 160 test, 2026-07-15) non era più stato aggiornato dopo le feature successive (es. backup email del 2026-08-14, che ha aggiunto suite non riflesse in quella riga); il conteggio odierno include quella crescita più le 3 nuove suite del campo Azienda (vedere bullet Feature sotto).
