...
**Un solo codice, un interruttore per installazione.** Ogni cliente ha la propria istanza (single-tenant), quindi il livello si sceglie con una variabile d'ambiente, ad esempio `NEXT_PUBLIC_PLAN=base` oppure `NEXT_PUBLIC_PLAN=completo` (il prefisso `NEXT_PUBLIC_` serve perché anche i componenti nel browser leggano il livello; il valore viene fissato al momento della pubblicazione, quindi il cambio richiede un nuovo deploy). Nessun fork.
