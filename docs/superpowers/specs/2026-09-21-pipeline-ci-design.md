...
| Corretto `package.json` (`"lint": "eslint ."`) e `eslint.config.mjs` (esclude `.claude/worktrees/**`) come parte di questo lavoro | Il primo fix è un prerequisito reale per la CI: senza di esso il passo di lint fallirebbe subito (`next lint` non esiste su Next 16). Il secondo, invece, non serve alla CI in sé — `.claude/worktrees/` è già in `.gitignore` e non arriva mai in un checkout fresco su GitHub Actions — ma migliora l'esperienza di sviluppo in locale (dove quella cartella esiste davvero) ed è comunque corretto tenerlo come rete di sicurezza |
